...
public class Simstate {
	...
	public void render(Graphics2D g2d, int xOffset, int yOffset, double scale) {
		...
		g2d.setStroke(new BasicStroke(2, BasicStroke.CAP_ROUND,
				BasicStroke.JOIN_ROUND)); // setting stroke of lines
		for (int i = 1; i < history.size(); i++) {
			...
			for (int j = 0; j < t.length; j++) {
				if (historyGradient) { // gradient paths
					...
					/*
					 * gradient function: -(2x)^2+1 -> gradient from c1 to c2
					 * where c1 is full at 0 and c2 is full at 1/2 the length of
					 * the array of points
					 */
					double mix = -1.0
							* (Math.pow(2.0 * (double) (history.size() - i)
									/ (double) history.size(), 2)) + 1.0;
					if (mix < 0)
						mix = 0;
					double mix2 = 1.0 - mix;
	...
	public void tickEuler(double t) {
		for (Planet p : thePlanets)
			p.updateAccelerationEuler();
		for (Planet p : thePlanets)
			p.updatePositionEuler(t);
		for (Planet p : thePlanets)
			p.updateVelocityEuler(t);
	}

	public void tickVerlet(double t) {
		for (Planet p : thePlanets)
			p.updatePositionVerlet(t);
		for (Planet p : thePlanets)
			p.updateAccelerationVerlet();
		for (Planet p : thePlanets)
			p.updateVelocityVerlet(t);
	}

	public void tickRK4(double dt) {
		// Initialisierung
		// aktuelles System erstellen
		State state = new State();
		// mit aktuellen Positionen und Geschwindigkeiten befuellen
		for (Planet p : thePlanets) {
			state.r.add(new Vector(p.getX(), p.getY()));
			state.v.add(new Vector(p.getvX(), p.getvY()));
		}
		// aktuelle Ableitung erstellen
		Derivative derivative = new Derivative();
		// Ableitungen der Positionen mit
		// Geschwindigkeiten des aktuellen Systems befuellen
		for (int i = 0; i < state.r.size(); i++) {
			derivative.dr.add(state.v.get(i));
		}
		// Ableitungen der Geschwindigkeiten mit
		// Beschleunigungen berechnet aus dem aktuellen System befuellen
		derivative.dv = acceleration(state);
		// rk4 Algorithmus
		Derivative a, b, c, d; // Ableitungen k1,k2,k3,k4
		...
		for (int i = 0; i < state.r.size(); i++) {
			drdt.add(a.dr.get(i)
					.add(b.dr.get(i).add(c.dr.get(i)).multiply(2.0))
					.add(d.dr.get(i)).divide(6.0));
			dvdt.add(a.dv.get(i)
					.add(b.dv.get(i).add(c.dv.get(i)).multiply(2.0))
					.add(d.dv.get(i)).divide(6.0));
	...
	public Derivative evaluate(State init, double dt, Derivative d) {
		State state = new State();
		// Das System um d * dt aktualisieren
		// state.r = init.r + d.dx*dt;
		// state.v = init.v + d.dv*dt;
		...
		// Ableitung von neuem System berechnen
		// output.dr = state.v;
	...
	// Beschleunigungsberechnung fuer ganze Systeme
		...
		for (int i = 0; i < state.r.size(); i++) {
			...
			for (int j = 0; j < state.r.size(); j++) {
				if (i != j) {
					...
					// a = (G*m)/(d*d)
					// ax = dx*(G*m*)/(d*d*d)
					acceleration.addS(new Vector(d.x, d.y).multiply(
							Sim.g * thePlanets.get(j).getM() / d.length3()));
				}
			}
			a.add(acceleration);
		}
	...
	public void addPlanet(double x, double y, double vX, double vY, double m,
			double p, Color c) {
		thePlanets.add(new Planet(x, y, vX, vY, m, p, c, this));
		history = new ArrayList<double[][]>();
		// recalculating initial acceleration
		for (Planet a : thePlanets)
			a.updateAccelerationVerlet();
	...
	public String[] getPlanetsAsText() {
		...
		for (int i = 0; i < thePlanets.size(); i++) {
			...
			r[i] = "addplanet " + p.getX() + " " + p.getY() + " " + p.getvX()
					+ " " + p.getvY() + " " + p.getM() + " " + p.getP();
		}
		return r;
	}
	...
	public void setHistoryGradient(boolean g) {
		historyGradient = g;
		if (historyGradient)
			System.out.println("gradient on");
		else
	...
	public void saveData(String fileName) {
		if (momentum.size() > 0) {
			...
			for (int i = 0; i < momentum.size(); i++) {
				s[i + 1] = time.get(i) + " " + momentum.get(i) + " "
						+ eKin.get(i) + " " + ePot.get(i) + " " + energy.get(i)
						+ " ";
				int newCompletion = (int) (100.0 * ((double) i / (double) momentum
						.size()));
	...
	public void printEnergyError() {
		if (collectData) {
			...
			System.out
					.println("  % energy error (initial -> last): "
							+ ((Math.abs(last) - Math.abs(initial)) /
								Math.abs(initial)));
